...
from ots2 import *
from .lib import restriction as restriction, test_config as test_config
...
class ParameterValidationTest(OTS2APITestBase):
    ...
    def _invalid_instance_or_table_name_op(self, client, table=None, error_code="", error_message=""):
        ...
        reserved_throughput = ReservedThroughput(
            CapacityUnit(restriction.MaxReadWriteCapacityUnit, restriction.MaxReadWriteCapacityUnit))
        # create_table
        try:
        ...
        # batch_write_row
        put_row_item = {"table_name": table_name,
                        "put": [PutRowItem(Condition(RowExistenceExpectation.IGNORE), {"PK": "x"}, {"COL": "x"})]}
        update_row_item = {"table_name": table_name, "update": [
            UpdateRowItem(Condition(RowExistenceExpectation.IGNORE), {"PK": "x"}, {'put': {"COL": "x"}})]}
        delete_row_item = {"table_name": table_name,
                           "delete": [DeleteRowItem(Condition(RowExistenceExpectation.IGNORE), {"PK": "x"})]}
    ...
    def test_update_row_columns_empty(self):
        ...
        # batch_write_row
        update_row_item = {"table_name": table_name,
                           "update": [UpdateRowItem(Condition(RowExistenceExpectation.IGNORE), primary_keys, {})]}
        ...
        try:
            ...
            self.assert_error(e, 400, "OTSParameterInvalid",
                              "No attribute column specified to update row #%d in table: '%s'." % (0, table_name))
    ...
    def test_invalid_instance_name(self):
        ...
        for instance_name in ["", "*", "#", "中文", "aa", "_aaa", "a###", "h***", "-aa", "aa-"]:
            ...
            self._invalid_instance_or_table_name_op(client, error_code="OTSParameterInvalid",
    ...
    def _valid_instance_or_table_name_op(self, client, table=None):
        ...
        # get_range
        consumed, next_start_primary_keys, rows = client.get_range(table_name, 'FORWARD', {"PK": INF_MIN},
                                                                   {"PK": INF_MAX})
        self.assert_consumed(consumed, CapacityUnit(1, 0))
        ...
        # update_row
        consumed = client.update_row(table_name, Condition(RowExistenceExpectation.IGNORE), {"PK": "x"},
                                     {'put': {"COL": "x1"}})
        self.assert_consumed(consumed, CapacityUnit(0, 1))
        # delete_row
        ...
        put_row_item = {"table_name": table_name,
                        "put": [PutRowItem(Condition(RowExistenceExpectation.IGNORE), {"PK": "x"}, {'COL': 'x'})]}
        update_row_item = {"table_name": table_name, "update": [
            UpdateRowItem(Condition(RowExistenceExpectation.IGNORE), {"PK": "x"}, {'put': {'COL': 'x1'}})]}
        delete_row_item = {"table_name": table_name,
                           "delete": [DeleteRowItem(Condition(RowExistenceExpectation.IGNORE), {"PK": "x"})]}
    ...
    def test_invalid_table_name(self):
        ...
        for table_name in table_list:
            ...
            self._invalid_instance_or_table_name_op(self.client_test, table_name, error_code="OTSParameterInvalid",
                                                    error_message="Invalid table name: '%s'." % table_name)
    ...
    def _invalid_column_name_test(self, table_name, primary_keys, columns_name, error_code="", error_message=""):
        ...
        # put_row
        try:
            self.client_test.put_row(table_name, Condition(RowExistenceExpectation.IGNORE), primary_keys,
                                     {columns_name: "x"})
            self.assert_false()
        except OTSServiceError as e:
            self.assert_error(e, 400, error_code, error_message)
        ...
        put_row_item = {"table_name": table_name, "put": [
            PutRowItem(Condition(RowExistenceExpectation.IGNORE), primary_keys, {columns_name: "x"})]}
        update_row_item = {"table_name": table_name, "update": [
            UpdateRowItem(Condition(RowExistenceExpectation.IGNORE), primary_keys, {'put': {columns_name: "x"}})]}
    ...
    def test_invalid_column_name(self):
        ...
        for column_name in column_name_list:
            self._invalid_column_name_test(table_name, primary_keys, column_name, error_code="OTSParameterInvalid",
                                           error_message="Invalid column name: '%s'." % column_name)
    ...
    def _valid_column_name_test(self, table_name, columns_name):
        ...
        self.assert_RowDataItem_equal(response, expect_response)
        # get_range
        consumed, next_start_primary_keys, rows = self.client_test.get_range(table_name, 'FORWARD', {"PK": INF_MIN},
                                                                             {"PK": INF_MAX}, [columns_name], None)
        self.assert_consumed(consumed, CapacityUnit(1, 0))
        self.assert_equal(next_start_primary_keys, None)
        self.assert_equal(rows, [])
        # put_row
        consumed = self.client_test.put_row(table_name, Condition(RowExistenceExpectation.IGNORE), pk,
                                            {columns_name: 'x'})
        self.assert_consumed(consumed, CapacityUnit(0, self.sum_CU_from_row(pk, {columns_name: 'x'})))
        # update_row
        consumed = self.client_test.update_row(table_name, Condition(RowExistenceExpectation.IGNORE), pk,
                                               {'put': {columns_name: "x1"}})
        self.assert_consumed(consumed, CapacityUnit(0, self.sum_CU_from_row(pk, {columns_name: "x1"})))

        # batch_write_row
        put_row_item = {"table_name": table_name,
                        "put": [PutRowItem(Condition(RowExistenceExpectation.IGNORE), pk, {columns_name: 'x'})]}
        update_row_item = {"table_name": table_name, "update": [
            UpdateRowItem(Condition(RowExistenceExpectation.IGNORE), pk, {'put': {columns_name: 'x1'}})]}
        write_batches = [put_row_item]
        response = self.client_test.batch_write_row(write_batches)
        expect_write_data_item = {"put": [BatchWriteRowResponseItem(True, "", "", "", CapacityUnit(0,
                                                                                                   self.sum_CU_from_row(
                                                                                                       pk, {
                                                                                                           columns_name: "x"})))]}
        ...
        write_batches = [update_row_item]
        response = self.client_test.batch_write_row(write_batches)
        expect_write_data_item = {"update": [BatchWriteRowResponseItem(True, "", "", "", CapacityUnit(0,
                                                                                                      self.sum_CU_from_row(
                                                                                                          pk, {
                                                                                                              columns_name: "x1"})))]}
        expect_response = [expect_write_data_item]
    ...
    def _invalid_pk_type_test(self, invalid_pk, is_range=None, error_code="", error_message="",
                              error_message_for_range=""):
        table_name = "table_test"
        ...
        for pk in valid_primary_keys.keys():
            ...
            # batch_write_row
            put_row_item = {"table_name": table_name,
                            "put": [PutRowItem(Condition(RowExistenceExpectation.IGNORE), primary_keys, {'COL': 'x'})]}
            update_row_item = {"table_name": table_name, "update": [
                UpdateRowItem(Condition(RowExistenceExpectation.IGNORE), primary_keys, {'put': {'COL': 'x'}})]}
            delete_row_item = {"table_name": table_name,
                               "delete": [DeleteRowItem(Condition(RowExistenceExpectation.IGNORE), primary_keys)]}
        ...
        def assert_get_range(table_name, inclusive_start_primary_keys, exclusive_end_primary_keys):
            try:
                self.client_test.get_range(table_name, 'FORWARD', inclusive_start_primary_keys,
                                           exclusive_end_primary_keys, [], None)
                self.assert_false()
            except OTSServiceError as e:
                self.assert_error(e, 400, error_code, error_message_for_range)

        if is_range == None:
            pk_schema, exclusive_primary_keys = self.get_primary_keys(restriction.MaxPKColumnNum, "STRING", "PK",
                                                                      INF_MAX)
            pk_schema, inclusive_primary_keys = self.get_primary_keys(restriction.MaxPKColumnNum, "STRING", "PK",
                                                                      INF_MIN)
            for pk in valid_primary_keys.keys():
                primary_keys = copy.copy(valid_primary_keys)
                primary_keys[pk] = invalid_pk
    ...
    def test_PK_double(self):
        ...
        double_value = 5.6
        self._invalid_primary_key_type_create_table("DOUBLE", error_code="OTSParameterInvalid",
                                                    error_message="DOUBLE is an invalid type for the primary key.")
        self._invalid_pk_type_test(double_value, error_code="OTSParameterInvalid",
                                   error_message="DOUBLE is an invalid type for the primary key.",
                                   error_message_for_range="DOUBLE is an invalid type for the primary key in GetRange.")

    def test_PK_boolean(self):
        """测试所有相关API中 PK为boolean的情况，期望返回OTSParameterInvalid"""
        boolean_value = True
        self._invalid_primary_key_type_create_table("BOOLEAN", error_code="OTSParameterInvalid",
                                                    error_message="BOOLEAN is an invalid type for the primary key.")
        self._invalid_pk_type_test(boolean_value, error_code="OTSParameterInvalid",
                                   error_message="BOOLEAN is an invalid type for the primary key.",
                                   error_message_for_range="BOOLEAN is an invalid type for the primary key in GetRange.")
    ...
    def test_table_not_exist(self):
        ...
        # update_row
        try:
            self.client_test.update_row(table_name, Condition(RowExistenceExpectation.IGNORE), {"PK": "x"},
                                        {'put': {"COL": "x"}})
            self.assert_false()
        except OTSServiceError as e:
            self.assert_error(e, 404, "OTSObjectNotExist", "Requested table does not exist.")
        ...
        batches = [(table_name, [{"PK": "x"}], [])]
        response = self.client_test.batch_get_row(batches)
        expect_row_data_item = RowDataItem(False, "OTSObjectNotExist", "Requested table does not exist.", "", None,
                                           None, None)
        expect_response = [[expect_row_data_item]]
        self.assert_RowDataItem_equal(response, expect_response)
        ...
        for item, key in zip(batch_write_items, resp_key):
            ...
            expect_write_data_item = BatchWriteRowResponseItem(False, "OTSObjectNotExist",
                                                               "Requested table does not exist.", "", None)
    ...
    def _column_restriction_test_except(self, columns, error_code="", error_message=""):
        ...
        put_row_item = {"table_name": table_name,
                        "put": [PutRowItem(Condition(RowExistenceExpectation.IGNORE), primary_keys, columns)]}
        update_row_item = {"table_name": table_name, "update": [
            UpdateRowItem(Condition(RowExistenceExpectation.IGNORE), primary_keys, {'put': columns})]}
        batches_list = [put_row_item, update_row_item]
        for item in batches_list:
            write_batches = [item]
            try:
    ...
    def test_column_type_INF_MIN(self):
        ...
        self._invalid_pk_type_test(INF_MIN, True, error_code="OTSParameterInvalid",
                                   error_message="INF_MIN is an invalid type for the primary key.")
        self._column_restriction_test_except({"COL": INF_MIN}, "OTSParameterInvalid",
                                             "INF_MIN is an invalid type for the attribute column.")

    def test_column_type_INF_MAX(self):
        """测试所有API中，主键和列的类型（get_range的start和end除外）为INF_MAX的情况，期望返回OTSParameterInvalid"""
        self._invalid_pk_type_test(INF_MAX, True, error_code="OTSParameterInvalid",
                                   error_message="INF_MAX is an invalid type for the primary key.")
        self._column_restriction_test_except({"COL": INF_MAX}, "OTSParameterInvalid",
                                             "INF_MAX is an invalid type for the attribute column.")
