...
class TableOperationTest(OTS2APITestBase):
    ...
    def test_update_CU_failed(self):
        """创建一个表，设置CU(1, 1)，操作验证CU，describe_table()确认设置成功，试图更新CU为(max + 1, max + 1)失败后，describe_table()获取CU为(1,1)， 操作验证CU"""
        table_name = 'table_update_CU_failed'
        table_meta = TableMeta(table_name, [('PK0', 'STRING'), ('PK1', 'STRING')])
        reserved_throughput = ReservedThroughput(CapacityUnit(1, 1))
        pk_dict_exist = {'PK0':'a', 'PK1':'1'}
        pk_dict_not_exist = {'PK0':'B', 'PK1':'2'}
        self.client_test.create_table(table_meta, reserved_throughput)
        self.wait_for_partition_load('table_update_CU_failed')
        ...
        try:
            reserved_throughput_new = ReservedThroughput(CapacityUnit(restriction.MaxReadWriteCapacityUnit + 1, restriction.MaxReadWriteCapacityUnit + 1))
            self.client_test.update_table(table_name, reserved_throughput_new)
            self.assert_false()
        except OTSServiceError as e:
